...
importScripts('helper.min.js');

this.onmessage = function(e) {

    //declare random pizza string
    var allRandomPizzas = e.data;

    //then add i pizza containers
    for (var i = 0; i < 100; i++) {
        //add a new section for the pizza, pizza'i'
        allRandomPizzas = allRandomPizzas + HTMLpizzaContainer.replace('%data%', i);

        //replace the name and ingredients of the new section
        allRandomPizzas = allRandomPizzas.replace('%pizzaName%', randomName());
        allRandomPizzas = allRandomPizzas.replace('%ingredientList%', makeRandomPizza());
    }

    //finally return as string and add to DOM in main.js
    postMessage(allRandomPizzas);
};